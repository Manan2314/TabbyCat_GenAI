...
// Fetch Clash Analysis
async function getClashAnalysis() {
    try {
       const response = await fetch(`${API_BASE_URL}/clash`);
        if (!response.ok) {
            throw new Error('Failed to fetch clash analysis');
        }
        const data = await response.json();
        document.getElementById('clash-results').innerText = data.clash_analysis || "No clash analysis available.";
    } catch (error) {
        console.error("Error fetching clash analysis:", error);
        document.getElementById('clash-results').innerText = "Error loading clash analysis.";
    }
}
...
async function getStrategyRecommendations() {
    try {
        ...
        if (!response.ok) {
            throw new Error('Failed to fetch strategy recommendations');
        }
        const data = await response.json();
        document.getElementById('strategy-results').innerText = data.strategy_recommendations || "No strategy recommendations available.";
    } catch (error) {
        console.error("Error fetching strategy recommendations:", error);
        document.getElementById('strategy-results').innerText = "Error loading strategy recommendations.";
    }
}
🔧 What you need to do next:
In your HTML (index.html or wherever results are displayed), add placeholders:

html
Copy
Edit
<div>
  <h3>Clash Analysis</h3>
  <pre id="clash-results">Click button to load clash analysis...</pre>
  <button onclick="getClashAnalysis()">Get Clash Analysis</button>
</div>

<div>
  <h3>Strategy Recommendations</h3>
  <pre id="strategy-results">Click button to load strategy...</pre>
  <button onclick="getStrategyRecommendations()">Get Strategy Recommendations</button>
</div>
